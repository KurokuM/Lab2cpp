...
double cosineAngle(const Vector3D& v1, const Vector3D& v2) {
    double produsulScalar = v1 * v2;
    double lungimeaVectorului = v1.length() * v2.length();

    if (lungimeaVectorului == 0) {
        // Evitarea împărțirii la zero
        return 0.0;
    }

    return produsulScalar / lungimeaVectorului;
}
